        ...
        <div class="flex-1 flex items-center justify-center py-20 px-6 relative z-10">
            <div class="max-w-2xl w-full flex flex-col items-center gap-12 text-center">
                ...
                <div class="flex flex-col items-center gap-4">
                    <h1 class="text-6xl max-sm:text-4xl font-black tracking-tighter leading-[1.05] m-0 bg-gradient-to-br from-accent-blue via-accent-purple to-accent-pink bg-clip-text text-transparent">
                        "Rust Fullstack"<br />"on Cloudflare!"
                    </h1>
                    <p class="text-base max-w-md m-0 text-text-secondary leading-relaxed">
                ...
                <Suspense fallback=move || view! {
                    <div class="flex items-center gap-2 px-4 py-2 rounded-full border border-border-subtle bg-surface-raised">
                        <Spinner />
                        <span class="text-xs text-text-muted">"Connecting..."</span>
                    </div>
                }>
                    {move || Suspend::new(async move {
                        let resp = health.await;
                        view! {
                            <div class="flex items-center gap-2.5 px-4 py-2 rounded-full border border-border-subtle bg-surface-raised">
                                <div class="w-1.5 h-1.5 rounded-full bg-accent-green shadow-[0_0_8px_var(--color-accent-green)] animate-pulse"></div>
                                <span class="text-xs font-medium text-accent-green">{resp.status.clone()}</span>
                                <span class="text-[10px] text-text-muted font-mono">"/api/health"</span>
                            </div>
                        }
                    })}
                </Suspense>
                ...
                <div class="grid grid-cols-2 max-sm:grid-cols-1 gap-3 w-full">
                    ...
                    <InfoCard title="Frontend">
                        <p class="m-0 text-xs text-text-secondary leading-relaxed">
                            "Edit "<code class="font-mono text-accent-blue">"frontend/src/main.rs"</code>
                            " — hot reload with Trunk"
                        </p>
                    </InfoCard>
                    <InfoCard title="Backend">
                        <p class="m-0 text-xs text-text-secondary leading-relaxed">
                            "Edit "<code class="font-mono text-accent-blue">"worker/src/lib.rs"</code>
                            " — add API routes with Axum"
                        </p>
                    </InfoCard>
                    <InfoCard title="Styling">
                        <p class="m-0 text-xs text-text-secondary leading-relaxed">
                            "Tailwind CSS v4 built in — just use utility classes in "<code class="font-mono text-accent-blue">"view!"</code>
                        </p>
                    </InfoCard>
                </div>
                ...
                <div class="w-full flex flex-col gap-5">
                    <p class="text-[11px] font-semibold tracking-[0.2em] uppercase text-center text-text-muted m-0">"Powered by"</p>
